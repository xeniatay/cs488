...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
        ...
        for (i = 0; i < group->numtriangles; i++) {
            ...
            if (mode & GLM_CELSHADING) {
              GLfloat *glnormal = &model->normals[3 * triangle->nindices[1]];
              set_celshading(glnormal);
            }
...
void set_celshading(GLfloat *glnormal) {
  ...
  double index = normal.dot(lightPos) * 15;
  if (index < 0) { index = 0; }
  int i = (int)index;
